...
- (void)startPullDownRefreshing {
    if (self.isPullDownRefreshed) {
        ...
        if (self.isLoadMoreRefreshed) {
            self.loadMoreView.frame = CGRectMake(0, [[UIScreen mainScreen] bounds].size.height-64, CGRectGetWidth(self.loadMoreView.frame), CGRectGetHeight( self.loadMoreView.frame));
        }
    }
}
...
- (void)endLoadMoreRefresing {
    if (self.isLoadMoreRefreshed) {
        ...
        loadMoreViewFrame.origin.y = self.scrollView.contentSize.height;
...
- (void)setRefreshState:(XHRefreshState)refreshState {
    switch (refreshState) {
        ...
        case XHRefreshStateNormal: {
            switch (self.pullDownRefreshViewType) {
                case XHPullDownRefreshViewTypeCircle:
                    self.refreshCircleContainerView.stateLabel.text = NSLocalizedString(title_pulldown_refresh, nil);
                    break;
                case XHPullDownRefreshViewTypeActivityIndicator:
                    break;
                case XHPullDownRefreshViewTypeCustom:
        ...
        case XHRefreshStateLoading: {
            if (self.pullDownRefreshing) {
                if (self.pullDownRefreshViewType == XHPullDownRefreshViewTypeCircle) {
                    self.refreshCircleContainerView.stateLabel.text = NSLocalizedString(msg_Load, nil);
                }
        ...
        case XHRefreshStatePulling:
            switch (self.pullDownRefreshViewType) {
                case XHPullDownRefreshViewTypeCircle:
                    self.refreshCircleContainerView.stateLabel.text = NSLocalizedString(title_Loosen_refresh, nil);
                    break;
                case XHPullDownRefreshViewTypeActivityIndicator:
                    break;
                case XHPullDownRefreshViewTypeCustom:
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    ...
    if ([keyPath isEqualToString:@"contentOffset"]) {
        ...
        if (self.isLoadMoreRefreshed && !self.noMoreDataForLoaded && !self.pullDownRefreshing && !self.loadMoreRefreshing) {
