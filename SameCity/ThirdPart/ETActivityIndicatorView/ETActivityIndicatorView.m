...
- (void)commonInit
{
    ...
    [colors addObject:UIColorFromRGB(0x9999cc)];
    [colors addObject:UIColorFromRGB(0x66cc99)];
    [colors addObject:UIColorFromRGB(0x99ccff)];
    [colors addObject:UIColorFromRGB(0xffcc66)];
    [colors addObject:UIColorFromRGB(0xff9999)];
}
...
-(void)startAnimating{
    ...
    if (!isAnimating){
        ...
        //add circles
        circleDelay = [NSTimer scheduledTimerWithTimeInterval:0.2 target: self
...
-(void)nextCircle{
    if (circleNumber<maxCircleNumber){
        ...
        circle.color = colors[circleNumber-1];
