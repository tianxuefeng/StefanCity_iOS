...
//        _actionButton = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemAction target:self action:@selector(actionButtonPressed:)];
        ...
        [button setImage:ImageWithName(@"navi_tab") forState:UIControlStateNormal];
        [button setImage:ImageWithName(@"navi_tab") forState:UIControlStateHighlighted];
        [button addTarget:self action:@selector(actionButtonPressedNew:) forControlEvents:UIControlEventTouchUpInside];
        UIBarButtonItem *barItem = [[UIBarButtonItem alloc] initWithCustomView:button];
        _actionButton = barItem;
    }
...
//        [items addObject:_nextButton];
    ...
    self.navigationItem.leftBarButtonItem = [UIBarButtonItem createBackBarButtonItemTarget:self action:@selector(back:)];
...
//        [navBar setBackgroundImage:[ImageWithName(@"navi_bg") adjustSize] forBarMetrics:UIBarMetricsDefault];
//        [navBar setBackgroundImage:[ImageWithName(@"navi_bg") adjustSize] forBarMetrics:UIBarMetricsLandscapePhone];
    }
}
...
#pragma mark -- modyfy by zengchao
...
#pragma mark -- add by zengchao 2014.4.5
- (void)actionButtonPressedNew:(UIBarButtonItem *)sender {
    
    UIView *bgView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 75, 60)];
    
    UIButton *saveBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    saveBtn.titleLabel.font = [UIFont systemFontOfSize:12];
    saveBtn.frame = CGRectMake(3, 0, 65, 30);
    [saveBtn setTitle:@"保存至手机" forState:UIControlStateNormal];
    [saveBtn setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    [saveBtn setTitleEdgeInsets:UIEdgeInsetsMake(0, 0, 0, -15)];
    [saveBtn setImageEdgeInsets:UIEdgeInsetsMake(0, -10, 0, 0)];
    [saveBtn setImage:ImageWithName(@"save_image_btn") forState:UIControlStateNormal];
    ...
    reportBtn.frame = CGRectMake(3, 32, 65, 30);
...
//    [reportBtn setTitleEdgeInsets:UIEdgeInsetsMake(0, 20, 0, -20)];
    [reportBtn setImageEdgeInsets:UIEdgeInsetsMake(0, -28, 0, 0)];
    [reportBtn setImage:ImageWithName(@"report_image_btn") forState:UIControlStateNormal];
    ...
    popView = [PopoverView showPopoverAtPoint:CGPointMake(290, 64) inView:self.view withContentView:bgView delegate:nil];
}
...
//    NSMutableDictionary *dic = [NSMutableDictionary dictionaryWithCapacity:1];
    ...
    [self showProgressHUDWithMessage:[NSString stringWithFormat:@"%@\u2026" ,@"提交举报中"]];
    [self showProgressHUDCompleteMessage:@"已举报"];
//    AFHTTPRequestOperationManager *netManager = [XFManagerCenter sharedManager].netManager;
//
//    [netManager GET:GET_SCHOOL_URL parameters:dic success:^(AFHTTPRequestOperation *operation, id responseObject) {
//        [self showProgressHUDCompleteMessage:@"已举报"];
...
- (void)savePhoto {
    id <MWPhoto> photo = [self photoAtIndex:_currentPageIndex];
    if ([photo underlyingImage]) {
        [self showProgressHUDWithMessage:[NSString stringWithFormat:@"%@\u2026" , @"正在保存"]];
...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo {
    [self showProgressHUDCompleteMessage: @"已保存"];
    [self hideControlsAfterDelay]; // Continue as normal...
}
