...
+(void)showInController:(UIViewController *)controller withText:(NSString *)text type:(LeafNotificationType)type{
    ...
    LeafNotification *tmpNotification = (LeafNotification *)[controller.view viewWithTag:9999];
    if (tmpNotification) {
        
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            
            LeafNotification *tmptmpNotification = (LeafNotification *)[controller.view viewWithTag:9999];
            tmptmpNotification.textLabel.text = text;
        });
        return;
    }
    
    LeafNotification *notification = [[LeafNotification alloc] initWithController:controller text:text];
    notification.tag = 9999;
