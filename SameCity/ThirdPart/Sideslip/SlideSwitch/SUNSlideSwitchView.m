...
//    _openUnselect = YES;
    ...
    for (UIView *tmpView in self.topScrollView.subviews) {
        [tmpView removeFromSuperview];
    }
    
    for (UIView *stmpView in self.rootScrollView.subviews) {
        [stmpView removeFromSuperview];
    }
    ...
    if (_slideSwitchViewDelegate && [self.slideSwitchViewDelegate respondsToSelector:@selector(slideSwitchView:didselectTab:)]) {
...
{
    if (!_shadowImageView) {
        _shadowImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, kHeightOfTopScrollView-3, 0, 36)];
    }
    _shadowImageView.frame = CGRectMake(60, kHeightOfTopScrollView-3, 0, 36);
//    _shadowImageView.contentMode = UIViewContentModeScaleAspectFit;
    ...
    for (int i = 0; i < [_viewArray count]; i++) {
        ...
        CGSize textSize = [NSString calculateTextHeight:[UIFont systemFontOfSize:kFontSizeOfTabButton] givenText:vc.title givenWidth:_topScrollView.bounds.size.width];
        
        if (textSize.height > kHeightOfTopScrollView) {
            textSize.height = kHeightOfTopScrollView;
        }
        ...
        [button setTag:i+100];
        if (i == 0) {
            _shadowImageView.frame = CGRectMake(kWidthOfButtonMargin, 36, textSize.width, 2);
            button.selected = YES;
        }
        ...
        [button addTarget:self action:@selector(selectNameButtonNoAnimation:) forControlEvents:UIControlEventTouchUpInside];
...
- (UIButton *)getTabButtonIndex:(int)index
{
...
- (void)selectNameButton:(UIButton *)sender
{
    ...
    [sender setTitleColor:self.tabItemSelectedColor forState:UIControlStateSelected];
    ...
    if (sender.tag != _userSelectedChannelID) {
        ...
        if (lastButton) {
            [lastButton setTitleColor:self.tabItemNormalColor forState:UIControlStateSelected];
    ...
    if (!sender.selected) {
        ...
        [UIView animateWithDuration:0.25 animations:^{
            [_shadowImageView setFrame:CGRectMake(sender.frame.origin.x, 36, sender.frame.size.width, _shadowImageView.frame.size.height)];
        } completion:^(BOOL finished) {
            if (finished) {
                //设置新页出现
                if (![_rootScrollView isDecelerating]) {
                    [_rootScrollView setContentOffset:CGPointMake((sender.tag - 100)*self.bounds.size.width, 0) animated:YES];
                }
                _isRootScroll = NO;
//                self.openUnselect = NO;
...
- (void)selectNameButtonNoAnimation:(UIButton *)sender
{
    if (!sender) {
        return;
    }
...
- (void)adjustScrollViewContentX:(UIButton *)sender
{
    ...
    else if (sender.frame.origin.x - _topScrollView.contentOffset.x < kWidthOfButtonMargin) {
...
//- (void)adjustScrollViewContentXNoAnimation:(UIButton *)sender
//{
//    //如果 当前显示的最后一个tab文字超出右边界
//    if (sender.frame.origin.x - _topScrollView.contentOffset.x > self.bounds.size.width - (kWidthOfButtonMargin+sender.bounds.size.width)) {
//        //向左滚动视图，显示完整tab文字
...
//        }
//        else {
//            [_topScrollView setContentOffset:CGPointMake(sender.frame.origin.x - kWidthOfButtonMargin, 0)  animated:NO];
//        }
//        
//    }
//}
